...
export default {
  data() {
    return {
      ...
      throttleDelay: 1000 //节流时间 防抖函数每过throttleDelay时间必定执行一次回调
  ...
  methods: {
    ...
    debounce(rowsCalc, columnsCalc) {
      ...
      //如果距离上一次执行onVirtualScrollFetch小于300ms
    ...
    onVirtualScrollFetch(rowsCalc, columnsCalc, now) {
      ...
      console.log("=====================================");
      console.log("开始执行onVirtualScrollFetch:", Date.now() - now);
      ...
      axios({
        ...
        (p) => {
          console.log("开始执行axios回调：", Date.now() - now);
          let that = this;
          ...
          if (area != -1) {
            ...
            console.log("计算新增区域完成时间：", Date.now() - now);
