...
export default {
  ...
  methods: {
    ...
    scrollEvent(e) {
      ...
      axios({
        method: "get",
        url: `/mdd/tabledata?id1=${this.startTop}&id2=${this.endTop}&id3=${this.startLeft}&id4=${this.endLeft}`,
        ...
        (p) => {
          //写在回调里 否则渲染先于axios执行
          // this.$store.commit("lcxTestState/change", p.data);
          // this.hot.loadData(
          //   this.$store.state.lcxTestState.nowData
          // );
          console.log(p.data);
      ...
      //   this.hot.loadData(
      //     this.tableData(this.startTop, this.endTop, this.startLeft, this.endLeft)
      //   );
    },

    // //模拟数据 移动滚轮时触发 返回相应的数据
    // tableData(startTop, endTop, startLeft, endLeft) {
    //   let data = [];
    //   let arr = [];

    //   for (let m = startTop; m <= endTop; m++) {
    //     for (let n = startLeft; n <= endLeft; n++) {
    //       arr.push(m + 1 + "行" + (n + 1) + "列");
    //     }
    //     data.push([...arr]);
    //     arr = [];
    //   }
  ...
  mounted() {
    ...
    axios({
      method: "get",
      url: `/mdd/tabledata?id1=${0}&id2=${8}&id3=${0}&id4=${8}`,
      cancelToken: new axios.CancelToken(function (c) {
        that.source = c;
      })
    }).then(
      (p) => {
        //写在回调里 否则渲染先于axios执行
        this.$store.commit("lcxTestState/change", p.data);
        this.hot.loadData(this.$store.state.lcxTestState.nowData);
      },
      (e) => {
        console.log(e);
      }
